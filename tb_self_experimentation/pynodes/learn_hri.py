...
class learn_hri:
    ...
    def __findClusters(self, X):
        ...
        self.cluster_center_ascend_distance = self.km.cluster_centers_[self.km.cluster_centers_[:,0].argsort()]


    def lowerDistance(self):
        # mean between centroids lower and best
        mean = (self.cluster_center_ascend_distance[1, 0] + self.cluster_center_ascend_distance[0, 0]) / 2
        return mean

    def bestDistance(self):
        # best centroid
        return self.cluster_center_ascend_distance[1, 0]

    def higherDistance(self):
        # mean between centroids lower and best
        mean = (self.cluster_center_ascend_distance[1, 0] + self.cluster_center_ascend_distance[2, 0]) / 2
        return mean
    ...
    def saveGraphic(self):
        ...
        plt.scatter(self.X[(self.y_km == 2), 0], self.X[(self.y_km == 2), 1],
                    ...
                    label='Far distance')
        # centroids
        plt.scatter(self.cluster_center_ascend_distance[:, 0], self.cluster_center_ascend_distance[:, 1],
                    s=50,
                    c='black',
                    marker='*',
                    label='Centroids')
        plt.axvline(self.lowerDistance())
        plt.text(self.lowerDistance(), 0, 'Lower', rotation=0)
